...
Keep the app resilient to UniFi API/schema changes. (A) Runtime version self-check: reads `/v1/info`, compares the controller's Network version to a tested range (`apiVersion.ts`, `TESTED_MIN`/`TESTED_MAX`), and surfaces it as an `API-VERSION` meta-finding. (B) Maintainer schema-drift CI: `tools/check-api-drift.ts` + `.github/workflows/api-drift.yml` weekly-diff the app's endpoint set against the latest published UniFi Network OpenAPI and open/auto-close a tracking issue on drift. (C) Updated the stale v9-era endpoint paths to their v10 names in `collect.ts` (`wlans`→`wifi/broadcasts`, `firewall-policies`→`firewall/policies`, `firewall-zones`→`firewall/zones`, `vpn-configs`→`vpn/servers`; `port-forwards`/`traffic-routes` have no v1 equivalent → backup-only). Response shapes derived from the v10.3.58 OpenAPI (not live hardware); the finding modules degrade gracefully where the v10 shape differs (verified by `fixture-local-api-v10.json` — no false alarms), and live device firmware now feeds advisory matching. Research + design: `docs/superpowers/specs/2026-07-03-api-currency-design.md`.
